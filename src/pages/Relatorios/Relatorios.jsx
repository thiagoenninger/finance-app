  ...
  const handleSaveReport = () => {
    ...
    try {
      ...
      const body = reportRows.map((row) => [
        row.pronac ?? "-",
        row.rubrica ?? "-",
        row.valorRubrica === "-" ? "-" : formatCurrencyBRLReport(row.valorRubrica),
        row.fornecedor ?? "-",
        row.numeroNF ?? "-",
        row.valorNotaFiscal === "-" ? "-" : formatCurrencyBRLReport(row.valorNotaFiscal),
      ])
                ...
                <tbody>
                  {reportRows.map((row, idx) => (
                    <tr key={idx}>
                      <td>{row.pronac}</td>
                      <td>{row.rubrica}</td>
                      <td>{row.valorRubrica === "-" ? "-" : formatCurrencyBRLReport(row.valorRubrica)}</td>
                      <td>{row.fornecedor}</td>
                      <td>{row.numeroNF}</td>
                      <td>{row.valorNotaFiscal === "-" ? "-" : formatCurrencyBRLReport(row.valorNotaFiscal)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
